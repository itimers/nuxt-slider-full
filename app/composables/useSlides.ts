...
export const useSlides = () => {
  const sliders = useState<Record<string, SliderInstance>>('slides-sliders', () => ({}));
  const isInit = useState<boolean>('slides-isInit', () => false);
  const activeInstance = useState<string | null>('slides-activeInstance', () => null);
  ...
  const setSlideWidth = (id: string, w: number) => { 
    getSlider(id).slideWidth = w; 
  };
  ...
  function stopAutoSlide(id: string, save = false) {
    ...
    if (inst.autoSlideTimer) { 
      clearTimeout(inst.autoSlideTimer); 
      inst.autoSlideTimer = null; 
    }
    inst.progressActive = false;
  }
  ...
  const setSliderContainerRef = (id: string, ref: HTMLElement | null) => { 
    getSlider(id).sliderContainerRef = ref; 
  };
  ...
  const doBoth = (fn: (id: string, ...args: any[]) => void, p: string, s: string, ...args: any[]) => { 
    fn(p, ...args); 
    fn(s, ...args); 
  };
  ...
  const getActiveSlideIndexInVisible = (id: string) => {
    const inst = getSlider(id);
    return inst.slides.filter(s => s.visibility).findIndex(s => s.active);
  };

  const getFilterDots = (id: string) => {
    const inst = getSlider(id);
    return inst.enableFilterDots ?? false;
  };

  const getSelectedSlides = (id: string) => {
    const inst = getSlider(id);
    return inst.slides.filter(s => s.selected);
  };

  const getFilterMode = (id: string) => {
    const inst = getSlider(id);
    return inst.filterMode ?? false;
  };

  const navigateToSlideId = (id: string, slideId: number) => {
    setCurrentSlideById(id, slideId);
  };

  const updateSlideZIndex = (id: string, slideId: number) => {
    const inst = getSlider(id);
    const slide = inst.slides.find(s => s.id === slideId);
    if (slide && inst.enableZIndex) {
      const maxZ = Math.max(...inst.slides.map(s => s.zIndex || 0));
      slide.zIndex = maxZ + 1;
    }
  };

  const toggleFilterMode = (id: string) => {
    const inst = getSlider(id);
    inst.filterMode = !inst.filterMode;
  };

  const initializeZIndex = (id: string) => {
    const inst = getSlider(id);
    if (inst.enableZIndex) {
      inst.slides.forEach((s, i) => {
  ...
  const setEnableFilterDots = (id: string, enabled: boolean) => {
    const inst = getSlider(id);
    inst.enableFilterDots = enabled;
  };

  const toggleQuickSelect = (id: string, slideId: number) => {
    const inst = getSlider(id);
    const slide = inst.slides.find(s => s.id === slideId);
    if (slide) {
